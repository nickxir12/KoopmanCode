    ...
    def __init__(
        ...
        if center is None:
            self.center = np.random.uniform(
                low=self.lift_low, high=self.lift_high, size=(self.Nrbf, self.Nstate)
            ).T
        else:
            self.center = center
...
class DerivativeLiftFunc:
    ...
    def Psi_s(self, s):
        ...
        if self.env_name.startswith("DampingPendulum"):
            theta, dtheta = s
            psi[self.Nstate] = (
                -self.g / self.l * np.sin(theta) - self.b * self.l * dtheta / self.m
            )
            psi[self.Nstate + 1] = (
                -self.g / self.l * np.cos(theta) * dtheta
                - self.b * self.l * psi[self.Nstate] / self.m
            )
        elif self.env_name.startswith("Pendulum"):
            theta, dtheta = s
            psi[self.Nstate] = 3.0 * self.g / (2.0 * self.l) * np.sin(theta)
            psi[self.Nstate + 1] = (
                3.0 * self.g / (2.0 * self.l) * np.cos(theta) * dtheta
            )
        elif self.env_name.startswith("CartPole"):
            ...
            psi[self.Nstate] = (self.g * sintheta - costheta * temp) / (
                self.l * (4.0 / 3.0 - self.mp * costheta**2 / self.mt)
            )
...
class data_collecter:
    def __init__(self, env_name) -> None:
        ...
        if self.env_name.startswith("DampingPendulum"):
        ...
        elif self.env_name.startswith("DoublePendulum") or self.env_name.startswith(
            "TwoLinkRobot"
        ):
            ...
            self.udim = 7
            self.reset_joint_state = np.array(self.env.reset_joint_state)
        elif self.env_name.startswith("Spirob"):  # MINE
